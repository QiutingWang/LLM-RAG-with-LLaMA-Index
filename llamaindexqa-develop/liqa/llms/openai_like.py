...
class OpenaiLLM(CustomLLM):
    ...
    model='gpt-4'
    api_base = "https://gf.nekoapi.com/v1/chat/completions"
    ...
    def give_response(self,input):
        ...
        response=requests.post(self.api_base, data=json.dumps(data), headers=self.headers)
    ...
    async def agive_response(self,input):
        ...
        async with sem:
            async with aiohttp.ClientSession(trust_env=True) as session:
                ...
                data = self.data
                data['messages'][-1]['content']=input
                async with session.post(self.api_base, data=json.dumps(data), headers=self.headers) as response:
                    return await response.text()
if __name__=='__main__':
  '''
capi_base http://103.177.28.196:8123/v1/chat/completions\
...
'''
  ...
  # 定义一个异步函数来调用Oapi_baseLM的acomplete方法，并打印执行时间
  async def main(i):
